...
const wordDisplay = document.querySelector(".word-display"); // Get the phrase display container
...
function loadQuizQuestion() {
    if (currentQuestionIndex >= quizQuestions.length) {
        ...
        wordDisplay.style.display = "block"; // Show phrase display again
        currentIndex = 0;
        updatePhrase();
        return;
    }
...
nextBtn.addEventListener("click", () => {
    if (currentIndex < phrases.length - 1) {
        ...
        wordDisplay.style.display = "none"; // Hide phrase display during quiz
        currentQuestionIndex = 0;
        loadQuizQuestion();
    }
});
